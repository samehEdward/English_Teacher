...
### Required to make the mic work in the APK — **[todo]**

The APK has **no speech recognition today** — Android System WebView does not implement
`webkitSpeechRecognition`, and `capacitor.plugins.json` is `[]`. Until this is run, the APK falls
back to `NullSttAdapter` and typed input:

    npm i @capacitor-community/speech-recognition
    npx cap sync android

No application code changes — `createSttAdapter()` picks the plugin up at runtime.
...
(Android 11+ package-visibility requirement; without it the recognizer is invisible to the app.)
